...
BOOL cutFileArch(NSString *path, int archType) {
    if (path == nil) {
        return NO;
    }
    if (archType == 1) {
        NSString *shellString = [NSString stringWithFormat:@"lipo \"%@\" -thin x86_64 -output \"%@\"", path ,path];
        executeCmdAndGetResult(shellString);
    } else if (archType == 2) {
        NSString *shellString = [NSString stringWithFormat:@"lipo \"%@\" -thin arm64 -output \"%@\"", path ,path];
        executeCmdAndGetResult(shellString);
    }
    return YES;
}
