...
- (void)mouseDragged:(NSEvent *)theEvent
{
    ...
    NSPoint where = [window convertRectToScreen:(NSRect){[theEvent locationInWindow],NSZeroSize}].origin;
	NSPoint origin = [window frame].origin;
	CGFloat deltaX = 0.0;
	CGFloat deltaY = 0.0;
	while ((theEvent = [NSApp nextEventMatchingMask:NSLeftMouseDownMask | NSLeftMouseDraggedMask | NSLeftMouseUpMask untilDate:[NSDate distantFuture] inMode:NSEventTrackingRunLoopMode dequeue:YES]) && ([theEvent type] != NSLeftMouseUp)) {
		@autoreleasepool {
            NSPoint now = [window convertRectToScreen:(NSRect){[theEvent locationInWindow], NSZeroSize}].origin;
...
- (void)_doInitialWindowSetup
{
	...
	_trafficLightButtonsTopMargin = 0.f;
...
- (void)_layoutTrafficLightsAndContent
{
	...
	if (!self.verticalTrafficLightButtons) {
		if (self.centerTrafficLightButtons) {
			...
			buttonOrigin = NSMaxY(titleBarFrame) - NSHeight(closeFrame);
		}
		buttonOrigin -= self.trafficLightButtonsTopMargin;
		...
		if (self.centerTrafficLightButtons) {
			...
			buttonOrigin = NSMaxY(titleBarFrame) - groupHeight;
		}
		buttonOrigin -= self.trafficLightButtonsTopMargin;
		closeFrame.origin.x = self.trafficLightButtonsLeftMargin;
